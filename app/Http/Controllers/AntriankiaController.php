...
class AntriankiaController extends Controller
{
    public function index()
    {
    	return view('layouts.module2.dashboard_kia');
    }
    ...
    public function get_antrian_kia()
    {
        ...
        $query = DB::table('tbl_antrian_kia')
                ->select('id', 'no_antrian_kia', 'status')
		...
		// ambil data hasil query
    		...
    		foreach($query as $index => $row) {
    			...
    			$data['no_antrian_kia'] = strval($row->no_antrian_kia);
    ...
    public function get_antrian_kia_selanjutnya()
    {
		// ambil tanggal sekarang
    	$tanggal = gmdate("Y-m-d", time() + 60 * 60 * 7);
		...
		// ambil data hasil query
    		$data =json_decode(json_encode($query), true);
		// buat variabel untuk menampilkan data
    		$no_antrian_kia = $data[0]['no_antrian_kia'];

		// tampilkan data
			$simpan = $no_antrian_kia;
			echo $simpan; // 00123
    	} 
		// jika data "no_antrian" tidak ada
    	else {
    ...
    public function get_jumlah_antrian_kia()
    {
		// ambil tanggal sekarang
    	$tanggal = gmdate("Y-m-d", time() + 60 * 60 * 7);
    ...
    public function get_sisa_antrian_kia()
    {
		// ambil tanggal sekarang
    	$tanggal = gmdate("Y-m-d", time() + 60 * 60 * 7);
