...
<body>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      var table = $('.tabel-antrian-akta').DataTable({
        "lengthChange": false,              // non-aktifkan fitur "lengthChange"
        "searching": false,                 // non-aktifkan fitur "Search"
        "ajax": '/get_antrian_akta',          // url file proses tampil data dari database
        
        // menampilkan data
        "columns": [{
            "data": "no_antrian_akta",
            "width": '250px',
      ...
      $('.tabel-antrian-akta tbody').on('click', 'button', function() {
        ...
        setTimeout(function() {
          responsiveVoice.speak("Nomor Antrian, " + data["no_antrian_akta"] + ", menuju, loket, AKTA KELAHIRAN", "Indonesian Female", {
            rate: 0.9,
            pitch: 1,
            volume: 1
          });
        ...
        $.ajax({
          ...
          url: '/admin/'+id+'/update_akta',          // url file proses update data ini ni l betulkn kang kles, cube tambahkn antrian sebiji lg
      ...
      setInterval(function() {
        $('#jumlah-antrian-akta').load("{{url('/get_jumlah_antrian_akta')}}").fadeIn("slow");
        $('#antrian-sekarang-akta').load("{{url('/get_antrian_akta_sekarang')}}").fadeIn("slow");
        $('#antrian-selanjutnya-akta').load("{{url('/get_antrian_akta_selanjutnya')}}").fadeIn("slow");
        $('#sisa-antrian-akta').load("{{url('/get_sisa_antrian_akta')}}").fadeIn("slow");
