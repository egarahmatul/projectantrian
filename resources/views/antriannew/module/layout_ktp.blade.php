...
<body>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      var table = $('.tabel-antrian-ktp').DataTable({
        "lengthChange": false,              // non-aktifkan fitur "lengthChange"
        "searching": false,                 // non-aktifkan fitur "Search"
        "ajax": '/get_antrian_ktp',          // url file proses tampil data dari database
      ...
      $('.tabel-antrian-ktp tbody').on('click', 'button', function() {
        ...
        setTimeout(function() {
          responsiveVoice.speak("Nomor Antrian, " + data["no_antrian_ktp"] + ", menuju, loket, K  T  P", "Indonesian Female", {
            rate: 0.9,
            pitch: 1,
            volume: 1
          });
        ...
        $.ajax({
          ...
          url: '/admin/'+id+'/update_ktp',          // url file proses update data ini ni l betulkn kang kles, cube tambahkn antrian sebiji lg
      ...
      setInterval(function() {
        $('#jumlah-antrian-ktp').load("{{url('/get_jumlah_antrian_ktp')}}").fadeIn("slow");
        $('#antrian-sekarang-ktp').load("{{url('/get_antrian_ktp_sekarang')}}").fadeIn("slow");
        $('#antrian-selanjutnya-ktp').load("{{url('/get_antrian_ktp_selanjutnya')}}").fadeIn("slow");
        $('#sisa-antrian-ktp').load("{{url('/get_sisa_antrian_ktp')}}").fadeIn("slow");
