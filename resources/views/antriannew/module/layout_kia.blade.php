...
<body>
    ...
    $(document).ready(function() {
      ...
      var table = $('.tabel-antrian').DataTable({
        ...
        "ajax": '/get_antrian_kia',          // url file proses tampil data dari database
      ...
      $('.tabel-antrian tbody').on('click', 'button', function() {
        ...
        setTimeout(function() {
          responsiveVoice.speak("Nomor Antrian, " + data["no_antrian_kia"] + ", menuju, loket, K I A", "Indonesian Female", {
            rate: 0.9,
            pitch: 1,
            volume: 1
          });
        ...
        $.ajax({
          ...
          url: '/admin/'+id+'/update',          // url file proses update data ini ni l betulkn kang kles, cube tambahkn antrian sebiji lg
      ...
      setInterval(function() {
        $('#jumlah-antrian').load("{{url('/get_jumlah_antrian_kia')}}").fadeIn("slow");
        $('#antrian-sekarang').load("{{url('/get_antrian_kia_sekarang')}}").fadeIn("slow");
        $('#antrian-selanjutnya').load("{{url('/get_antrian_kia_selanjutnya')}}").fadeIn("slow");
        $('#sisa-antrian').load("{{url('/get_sisa_antrian_kia')}}").fadeIn("slow");
